...
public interface ClubRepository extends JpaRepository<Club, String> {
    ...
    @Query("select c from Club c where c.id not in (select i.club.id from Integration i where i.etudiant.id = ?1)")
    List<Club> findAllClubsWhereUserNotJoined(String studentId);
}